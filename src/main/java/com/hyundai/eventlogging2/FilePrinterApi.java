...
public class FilePrinterApi {
    public void print(WebhookDto dto) throws IOException {

		Config config = Config.getConfig();

		String outFormat = config.getString("webhook.api.message.format","[#time][#projectname] [#level] #message");
		...
		Config.getConfig().getString("webhook.api.message.date.format", "yyyy-MM-dd HH:mm:ss")));
		String dir = config.getString("webhook.api.file.path", "out/outputFile1");
	...
	private void printToday(String str) throws IOException {
		...
		String dot = ".";
		String suffix = StringUtils.formatDate(currentTimestamp, config.getString("webhook.api.file.rolling.suffix", "yyyyMMdd"));
		String dir = config.getString("webhook.api.file.path", "out/outputFile1");
